...
const IconButton: React.FC<IconButtonProps> = ({ icon: Icon, onClick, className, ariaLabel }) => (
  ...
  >
    <Icon size={20} className="text-white" /> {/* Cor dos ícones alterada para branco */}
  </button>
);
...
export default function ChatInterface() {
  ...
  // Carregar email e token dos parâmetros da URL
  useEffect(() => {
    const emailFromParams = searchParams.get('email') || '';
    const tokenCryptFromParams = searchParams.get('tokenCrypt') || '';
    setUserEmail(emailFromParams);
    setToken(tokenCryptFromParams);

    // Adicionar mensagem de boas-vindas apenas na inicialização, se não houver mensagens
    if (emailFromParams && messages.length === 0) {
      const greeting = getGreetingMessage(emailFromParams);
      setMessages([{ sender: 'assistant', text: greeting, isWelcome: true }]);
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (conversationsPopupRef.current && !conversationsPopupRef.current.contains(event.target as Node)) {
  ...
  const getGreetingMessage = (email: string) => {
    const hour = new Date().getHours();
    let greeting = '';
    if (hour < 12) {
      greeting = t('goodMorning', { defaultMessage: 'Good morning' });
    } else if (hour < 18) {
      greeting = t('goodAfternoon', { defaultMessage: 'Good afternoon' });
    } else {
      greeting = t('goodEvening', { defaultMessage: 'Good evening' });
    }
    const userName = email.split('@')[0];
    return `${greeting}, ${userName}. ${t('howCanIHelp', { defaultMessage: 'How can I help you today?' })}`;
  };
  ...
  const sendMessage = async () => {
    ...
    if (input.trim()) {
      newMessages.push({ sender: 'user', text: input } as Message);
    }

    if (selectedFiles.length > 0) {
      const fileNames = selectedFiles.map((file) => file.name).join(', ');
      newMessages.push({ sender: 'user', text: `Uploaded files: ${fileNames}` } as Message);
    }
    ...
    try {
      const response = await fetch('http://localhost:5000/api/dados', {
        ...
        body: JSON.stringify({ nome: input || 'File upload', token: token }),
      });
      ...
      const outputValue = result.result?.output || t('noResponse', { defaultMessage: 'No response received' });
      ...
      const intervalId = setInterval(() => {
        if (visibleText.length < fullText.length) {
          ...
          setMessages([...newMessages, { sender: 'assistant', text: visibleText } as Message]);
        } else {
  ...
  const handleLogout = async () => {
    try {
      ...
      localStorage.removeItem('conversations'); // Limpar conversas ao fazer logout
      ...
      <main className="flex-1 flex flex-col w-full">
        <header className="bg-gray-900 shadow p-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            />
            {/* Pop-up de conversas anteriores */}
              ...
              >
                <div className="p-4">
                  ...
                  {conversations.length === 0 ? (
                    <p className="text-gray-500 text-sm">{t('noConversations', { defaultMessage: 'No previous conversations' })}</p>
                  ) : (
          ...
          <div className="relative flex items-center gap-2" ref={userMenuRef}>
            <User size={20} className="mr-2 text-white" /> {/* Cor do ícone de usuário ajustada */}
            <button
